...
在同一任务上连续启动失败 2 次后，调度器会以最后一个错误为原因自动阻塞该任务，防止因配置文件不存在、工作区无法挂载等原因导致的反复启动。
