...
选择 MoA 预设后，参考模型会先生成分析，聚合模型负责生成最终回复和发起工具调用。
...
7. 工具结果写回会话后，如果还需要继续生成回复，Hermes 会按同一流程再运行一轮
...
`enabled: false` 会关闭该预设的参考模型扇出，只保留聚合模型作为普通模型运行。
...
顶层 Agent 调用 `delegate_task` 时默认后台运行。工具调用立即返回运行状态、`delegation_id` 等信息；子 Agent 完成后，结果会自动作为内部消息回到对话。

批量委派的全部子 Agent 完成后，会将所有结果按输入顺序排列合并为一条消息回到对话。
