...
运行定时任务时，会禁用 cron 管理工具，避免递归创建更多定时任务造成调度循环。
...
任务存储在 `~/.hermes/cron/jobs.json`，Hermes 会把运行输出保存到 `~/.hermes/cron/output/{job_id}/{timestamp}.md`。
...
默认投递内容包含定时任务的头尾内容，以便用户知晓其来自于定时任务：
...
> 注意：`context_from` 读取的是上游任务「最近一次已完成输出」，不会等待同一个 tick 中仍在运行的上游任务。如果上游本次运行还没结束、下游已经触发，下游会拿到上游上一次完成的结果。需要强依赖同一批数据时，请把上下游任务错开足够长的时间，或把多个步骤合并到同一个 cron prompt / 脚本里串行执行。
