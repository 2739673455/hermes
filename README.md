...
Kanban dispatcher 只负责推进已经结构化的 task：哪些 task 已经 ready、应该启动哪个 assignee、worker 是否完成或失败。它不会理解“帮我做一次市场调研并写成 memo”这种高层目标，也不会自动决定该拆成几步、先让谁研究、谁写稿、谁审查。因此，当用户目标跨越多个阶段或多个专业 profile 时，需要一个专门的控制面角色来做规划和路由，这就是 orchestrator。

Orchestrator 是 Kanban 里的“编排者”角色。它不负责直接执行具体任务，而是把用户的高层目标翻译成任务图：把目标拆成 task，给每个 task 指派合适的 profile，建立依赖关系，并在下游任务完成后做汇总、判断是否需要继续拆分。
...
5. 阅读结果，汇总给用户，或继续创建下一批 follow-up task。

这个角色的价值不在于“更聪明地执行任务”，而在于把一次含糊的高层请求变成可恢复、可审计、可并行推进的 Kanban 状态。没有 orchestrator，用户或某个 worker 就必须手动承担拆解、路由和依赖管理；有了 orchestrator，这部分协调工作可以被固定在一个受约束的 profile 里。
