...
- **多模型协作**：通过 MoA 预设聚合多个参考模型，由聚合模型生成最终回复并正常调用工具。
...
1. 检查并准备环境
   ...
   - 检查网络连通性以及 curl、xz-utils、ripgrep、ffmpeg 等系统依赖；缺失时会尝试自动安装或提示手动安装
2. 拉取或更新 Hermes 源码
   - 普通用户安装时拉取或更新 Hermes 源码到 `~/.hermes/hermes-agent`
3. 安装依赖
   - 创建 Python 虚拟环境并安装 Python 依赖
   - 安装 Node.js 依赖、浏览器工具依赖和 TUI 依赖
   - 没有可用系统浏览器且未跳过浏览器安装时，会尝试安装 Playwright Chromium
4. 创建命令入口
   - 在 `~/.local/bin/hermes` 创建 `hermes` launcher
5. 初始化数据目录和配置
   - 初始化数据目录。默认是 `~/.hermes`
   - 仅在文件不存在时创建 `config.yaml`、`.env`、`SOUL.md`，并保留已有配置
6. 同步技能和平台组件
   - 同步内置技能
...
├── SOUL.md        # 主 Agent 的身份文件，会拼接到系统提示词开头部分
├── skills/        # 技能
├── cron/          # 定时任务
...
MoA 是 Hermes 的多模型协作能力，用于让多个模型从不同角度分析同一个任务，再汇总成一个最终回复。

选择 MoA 预设后，会先让参考模型生成分析，然后再让聚合模型生成最终回复并发起工具调用。
...
`/model <预设名> --provider moa` 将当前会话模型切换为指定 MoA 预设。
...
`/moa <提示词>` 对当前提示词使用默认 MoA 预设，执行后恢复原模型。

## 4.2 工作原理
当会话当前模型是某个 MoA 预设时，Hermes 在每次需要模型回复时按以下流程运行：

1. 读取当前 MoA 预设，得到参考模型、聚合模型和生成参数
2. 先运行参考模型。参考模型只负责给出分析，不会调用工具，也不会成为用户可见回复
3. 参考模型接收裁剪后的会话文本，只包含用户和助手文本，不包含 Hermes 系统提示词、工具定义和工具调用记录
4. 将参考模型输出作为私有上下文交给聚合模型；这些内容不会写入可见会话记录
5. 聚合模型接收普通 Hermes 上下文、工具定义和参考模型输出，负责生成用户可见回复和工具调用
6. 如果聚合模型发起工具调用，Hermes 正常执行工具
7. 工具结果写回会话后，如果还需要继续生成回复，会按同一流程再运行一轮

提示词缓存行为：
- 选择 MoA 预设不会改写已有历史、系统提示词或工具定义
- 稳定的会话前缀仍按普通模型调用缓存
- 传给聚合模型时，参考模型输出追加在最新用户消息尾部，只影响本轮新增内容
- 切换到或离开 MoA 预设，与普通 `/model` 切换的缓存影响相同
- MoA 的主要额外开销是每轮多出来的参考模型调用
...
将 `enabled` 设为 `false` 会关闭该预设的参考模型扇出，只保留聚合模型作为普通模型运行。

`reference_max_tokens` 可选，用于限制参考模型的输出 token。
...
2. 确定压缩边界
   ...
   - 对齐 `tool_call` / `tool_result` 边界，避免拆散工具调用组
3. 生成结构化摘要
   - 由辅助模型(`auxiliary.compression`)生成摘要
...
5. 迭代重压缩
   - 再次触发压缩时，会把前一次摘要连同指令一起传给 LLM，要求其更新摘要而非从头摘要
6. 摘要模型要求
...
Hermes 会把 `required_environment_variables` 中已配置的环境变量自动传递到 `execute_code` 和 `terminal` 沙箱，Skill 的脚本可以直接使用对应环境变量。

### 8.1.2 输出与媒体传递
Skill 或 Agent 需要发送图片、音频、视频或文档时，可以在回复里写本地文件路径。Gateway 会识别这些路径，把对应文件作为聊天平台附件发送，用户看到的是附件，不会看到路径文本。
...
内部投递指令可以放在同一条回复的任意位置。Gateway 会在投递前移除这些指令，用户不会看到它们。

同一条回复可以同时使用两个指令：音频按语音消息发送，图片按文件附件发送。
...
### 8.5.2 触发方式
Agent 创建和更新 Skill 的触发方式包括：

| 触发方式                | 行为                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| 系统提示词              | 要求 Agent 在复杂任务、困难修复或发现非平凡流程后保存 Skill；如果已加载 Skill 有问题，要立即修补 |
| `skill_manage` 工具说明 | 工具 schema 会明确何时 `create`、`patch`、`edit`、`write_file`、`delete`                         |
| 后台 review             | 工具调用迭代数达到 `skills.creation_nudge_interval`（默认 15）后，回合结束时 fork 后台 review    |
| `/learn <anything>`     | 从目录、URL、文字材料或最近工作流提炼并创建 Skill                                                |
...
- 创建的新 Skill 写入 `~/.hermes/skills/`
- 修改已有 Skill 时，会在找到该 Skill 的位置就地修改
- 如果外部 Skill 目录对 Hermes 进程可写，`skill_manage` 可以修改其中的文件
- 需要只读共享 Skill 时，使用文件系统权限约束

## 8.6 Curator (技能维护)
每次 Agent 解决新问题并保存技能时，都会把新 Skill 写入 `~/.hermes/skills/`。若不进行维护，最终可能会出现数十个范围狭窄的近似重复项，污染技能目录并浪费 token。

为防止通过自我改进循环创建的技能无限堆积，Curator 在后台维护 Agent 创建的技能：
...
### 8.6.1 运行机制
Curator 由空闲检查触发。Hermes 启动或 Gateway 后台 tick 时，会检查是否满足如下启动条件：

- `curator.enabled` 为 `true`
- Curator 未通过 `hermes curator pause` 暂停
- 距离上次运行已经超过 `interval_hours`，默认 168 小时（7天）
- Agent 已经空闲超过 `min_idle_hours`，默认 2 小时

首次安装或还没有 `last_run_at` 记录时，Curator 不会运行；Hermes 会写入当前时间作为基准，并推迟一个时间间隔。

用户也可以通过 `hermes curator run` 手动触发。
...
2. **LLM Review**：启动辅助模型(`auxiliary.curator`)
...
### 8.6.5 Curator 处理范围
如果某个 Skill 不属于内置技能，也不是 Skills Hub 安装的技能，Curator 就会处理它。
...
- 不会被转换状态，保持 `active` 状态
- 不会被 Curator 运行 LLM 审查
- 不会被 `skill_manage(action="delete")` 删除
- 允许 `patch` 和 `edit`，因此 Agent 仍可修补已固定技能的内容
...
每次 Curator 运行后，Hermes 都会在 `~/.hermes/logs/curator/` 下写入一个带时间戳的目录：
...
## 9.4 Shell Hook 示例：对话结束、请求审批或澄清完成时弹窗提示
...
# 11. 持久记忆
Hermes Agent 拥有持久记忆。记忆有容量上限，且由 Agent 自行维护，用于跨会话保存用户偏好、项目环境和经验教训。
...
- 会话开始时，Hermes 会把这两个文件注入系统提示词
- Agent 在会话中通过 `memory` 工具新增、替换或删除记忆后，Hermes 会立即写入磁盘，但当前会话不立刻生效
- 新记忆在下一个会话生效，以保持 LLM prefix cache 稳定
...
没有 `read` 动作。Hermes 会自动将记忆内容注入系统提示词，Agent 在会话中能直接看到。
...
新增条目超过上限时，`memory` 工具会返回错误，并附带当前条目和容量信息。Agent 应先删除或整合旧条目，使用 `replace` 合并相关记忆，再添加新条目。
...
## 12.3 `SOUL.md` 与个性
`SOUL.md` 定义 Hermes Agent 的身份与个性。Hermes 会把它拼接到系统提示词开头部分。
...
# 14. Profile
Profile 用于创建并运行多个独立的 Agent，每个 Agent 都有独立的配置、会话、记忆、技能、定时任务、状态数据库和 Gateway。
...
注入前，Hermes 会把每个上游输出截断至 8,000 字符（超出部分以 `[... output truncated ...]` 标记），避免 prompt 过度膨胀。

Agent 可以通过 `cronjob` 工具的 `context_from` 参数设置上游任务；也可以人工在 Dashboard 中设置 `context_from`。

`context_from` 读取上游任务最近一次已完成输出，不等待同一个 tick 中仍在运行的上游任务。上游本次运行还没结束、下游已经触发时，下游会拿到上游上一次完成的结果。需要强依赖同一批数据时，把上下游任务错开足够长的时间，或把多个步骤合并到同一个 cron prompt / 脚本里串行执行。
...
顶层 Agent 调用 `delegate_task` 时默认后台运行。工具调用立即返回运行状态、`delegation_id` 等信息；子 Agent 完成后，会把结果自动作为内部消息送回对话。

批量委派的全部子 Agent 完成后，会把所有结果按输入顺序排列，并合并为一条消息回到对话。
...
### 17.3.4 Comment
Comment 是人类或 Agent 在 task 上追加的持久消息，也是 Kanban 的跨 Agent 交接协议，对应 `task_comments` 表。启动或重新启动 worker 时，会读取完整评论串。人类可以通过评论补充要求、回答 worker 的问题、纠正方向；Agent 也可以通过评论留下中间发现、交接说明。

### 17.3.5 Event
Event 是 Kanban 的任务审计日志，对应 `task_events` 表。它记录 task 的生命周期变化、人工编辑和 worker 执行遥测。

常见事件包括创建、认领、完成、阻塞、归档、改派、编辑、心跳、崩溃、超时和重试放弃等。事件用于查看任务历史、排查 worker 执行问题和还原任务状态变化。

### 17.3.6 Workspace
Task 绑定的 workspace 是其 worker 执行时所在的目录。Kanban 支持如下 workspace 类型：
...
### 17.3.9 Tenant
Tenant 是 task 的可选分组标识，对应 `tasks.tenant` 字段，用于在同一 board 内按业务上下文标记和过滤任务。

设置 Tenant 后：

- `kanban_list`、批量 specify / decompose 可按 tenant 过滤任务
- worker 进程会收到 `HERMES_TENANT`
- `kanban_show()` 返回的上下文中会包含 `Tenant: <name>`
- worker 或 orchestrator profile 创建子任务时默认继承 `HERMES_TENANT`
- 分解器创建的子任务继承原始 task 的 tenant

Tenant 是软分组标识，不会自动绑定 workspace，也不会自动隔离记忆或权限。
...
5. 人工介入
   ...
   Worker 遇到不确定的情况时，把 task 置为 `blocked`，并在任务评论中附上疑问。用户或其他 profile 通过评论回复并 unblock 任务。调度器重新启动 worker 后，任务评论会成为下一次 worker 的上下文。
...
- Decomposer：Gateway 内的辅助 LLM 流程，读取 `triage` task 和可用 profile 描述，创建子任务和依赖关系
- Orchestrator Profile：是一个承接 root task 的普通 Hermes profile，负责读取子任务结果，完成总体验收和汇总

分解后，原始 `triage` task 会变成 root task，并指派给 `kanban.orchestrator_profile`。所有子任务完成后，root task 推进到 `ready`，调度器再启动编排 profile 处理汇总。如果整体目标已经完成，编排 profile 完成 root task；如果还缺步骤，编排 profile 继续追加 task 或阻塞等待输入。

触发方式：

- 自动：调度器每个 tick 自动处理 `triage` task
...
创建 `orchestrator` profile，并把它配置为编排器：
...
调度器启动的 Kanban worker 会自动获得当前任务所需的 Kanban 生命周期工具。当需要让 `orchestrator` profile 在普通对话中主动管理看板时，需要手动启用 `kanban` toolset：
