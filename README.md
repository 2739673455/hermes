...
## 17.5 Orchestrator Profile
### 17.5.1 Orchestrator 的职责
Orchestrator 是 Kanban 里的“编排者”角色。它不负责直接执行具体任务，而是负责把用户的高层目标拆成 task，给每个 task 指派合适的 profile，建立依赖关系，并在下游任务完成后做汇总、判断是否需要继续拆分。
...
### 17.5.2 Orchestrator Profile 的约束
PDF 里把一个行为良好的 orchestrator 归纳为三个属性：
...
### 17.5.3 当前使用方式：直接聊天或作为 Kanban 总任务 assignee
...
## 17.6 Dispatcher Design：调度器设计
### 17.6.1 Dispatcher 职责
dispatcher 的职责是把 board 中“已经满足运行条件”的任务交给对应 profile worker，而不是做智能规划。它不理解任务内容，不自动选择最佳 assignee，也不负责拆解复杂目标。

拆解、路由、优先级策略和“该让谁做”这类判断，应该由人类、orchestrator profile、脚本或更上层自动化完成。dispatcher 只负责把已经写进 board 的状态可靠地推进下去。

### 17.6.2 四个动作：recompute ready、atomic claim、spawn worker、stale recovery
一次 dispatcher tick 主要做四类动作：
...
2. **recompute ready**：重新计算哪些 `todo` 任务可以进入 `ready`。没有父任务，或所有父任务都已经 `done` / `archived` 的任务，会被推进到 `ready`。
3. **atomic claim**：扫描 `ready` 且 `claim_lock IS NULL` 的任务。没有 `assignee` 的任务只记录为 skipped；可启动任务通过 compare-and-swap 式 SQL 更新认领，成功后变成 `running`，并写入 `claim_lock` 和 `claim_expires`。
...
当前实现还会记录 `task_runs`：每次认领都会产生一条运行记录，用来保存这次运行的 profile、claim、worker PID、开始 / 结束时间、summary、metadata 和 error。

### 17.6.3 SQLite 并发正确性
认领任务不能用“先查再改”，否则并发 tick、重复触发或恢复过程可能让同一个 task 被启动两次。Hermes 使用 SQLite 写事务和 compare-and-swap 式更新来保证同一时刻只有一个认领者成功。
...
当前实现还会在认领前再次检查父任务状态：如果某个 ready task 的父任务并非全部完成，会把它退回 `todo`，避免因为并发修改或手工 SQL 造成依赖绕过。

### 17.6.4 当前实现：Gateway 内置 dispatcher
当前实现中，dispatcher 默认运行在 Gateway 内部，按配置的时间间隔执行 tick。这样只要 `hermes gateway` 在运行，ready task 就会被自动拾取。

旧的独立 daemon 形态已经不再是推荐入口。实际使用时，一般通过 Gateway 承载 dispatcher；CLI 的 `hermes kanban dispatch` 更适合手动触发一次调度或调试。

dispatcher 启动 worker 时，会把当前 task、board、数据库路径、workspace 和 claim lock 等信息写入环境变量，确保 worker 回写到同一个 board。

### 17.6.5 失败、重试和 circuit breaker
...
为了避免坏任务无限重试，当前实现维护连续失败计数。spawn failure、timeout、crash 等非成功结果会增加失败计数；任务成功完成后才会清零。超过失败阈值后，任务会被自动阻塞，并保留最后一次错误信息，等待人类或 orchestrator profile 介入。
