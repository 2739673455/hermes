...
| `/subgoal <text>`       | 在循环进行中向活动目标追加一个用户自定义条件，只有原始目标和所有子目标都满足时，才会把目标标记为完成 |
| `/sessions`             | 查看和管理会话                                                                                       |
...
# 6. Toolsets
工具是扩展 Agent 能力的函数。Hermes 按逻辑将工具组织为工具集。
...
Gateway 会在投递前移除该指令，用户不会看到它。Gateway 会把同一响应中的所有媒体路径按文档附件方式发送。
...
- **本地创建，就地更新**：Agent 创建的新 Skill 写入 `~/.hermes/skills/`；Agent 会在找到的位置修改已有 Skill
...
- **路径可选**：Hermes 会静默跳过不存在的外部目录，适合跨机器共享配置
...
- **跳过缺失 Skill**：捆绑包会加载能解析的 Skill，并提示跳过了哪些条目
- **跨界面可用**：交互式 CLI、TUI、Dashboard Chat 和 Gateway 平台都可以使用捆绑包
- **不会修改系统提示词**：调用捆绑包时会生成一条新的用户消息，不会使 prompt 缓存失效
...
Agent 可以通过 `skill_manage` 工具创建、修改和删除自己的技能。这是 Agent 的「程序记忆」：当它找到一个非平凡的工作流时，会把该方法保存为 Skill 以供将来复用。

Agent 创建和更新 Skill 的入口包括：
...
| 后台 review agent       | 当工具调用迭代数达到 `skills.creation_nudge_interval` 后，回合结束时会 fork 后台 review，回顾本轮对话并更新 Skill library         |
...
## 8.6 Curator (技能维护)
每次 Agent 解决新问题并保存技能时，都会把新 Skill 写入 `~/.hermes/skills/`。若没有维护，最终可能会出现数十个范围狭窄的近似重复项，污染技能目录并浪费 token。
...
用户也可以通过 `hermes curator run` 手动触发；手动运行会跳过时间间隔和空闲时长门槛，但仍要求 `curator.enabled` 不是 `false`。
...
### 8.6.5 Curator 处理范围
如果某个 Skill 不属于 bundled 内置技能，也不是 Skills Hub 安装的技能，Curator 就会处理它。
...
- Curator 自动状态转换会跳过它，并保持 `active` 状态
...
| `blocked`   | 当前无法继续执行，需要人工输入、问题修复或外部条件变化            |
| `done`      | 已完成，是正常完成终态                                            |
