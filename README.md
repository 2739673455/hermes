...
hermes config get section.key        # 读取解析后的配置值
hermes config unset section.key      # 删除用户设置的配置值
```
...
| 模式     | 行为                                                                                                     |
| -------- | -------------------------------------------------------------------------------------------------------- |
| `smart`  | 默认使用此模式。使用辅助模型评估风险；低风险命令仅批准当前一次，高风险命令直接拒绝，不确定时请求人工审批 |
| `manual` | 所有被标记为危险的命令都请求人工审批                                                                     |
| `off`    | 跳过危险命令审批，效果等同于 `--yolo`                                                                    |

`approvals.deny` 匹配的命令始终会被拒绝，即使启用了 `yolo` 或 `approvals.mode: off`。
...
moa:
  ...
  presets:
    default:
      ...
      fanout: per_iteration         # per_iteration：默认，每次工具迭代运行；user_turn：每轮用户消息运行一次并复用
...
hermes sessions archive --older-than 30      # 隐藏 30 天前的会话，不删除消息和搜索数据
...
## 5.2 会话存储
Hermes 使用 SQLite 数据库 `~/.hermes/state.db` 保存会话元数据、完整消息历史、搜索索引等。
...
## 5.5 会话导出
Hermes 可以把会话导出为 JSON 记录、可读文档、独立网页或 Agent Trace 文件。

| 格式         | 导出内容                                      |
| ------------ | --------------------------------------------- |
| `jsonl`      | 每个会话一条完整 JSON 记录，默认格式          |
| `md` / `qmd` | 每个会话一个 Markdown 或 Quarto 文件          |
| `html`       | 包含会话内容的独立 HTML 文件                  |
| `trace`      | 供 Hugging Face Agent Trace Viewer 使用的记录 |

| 选项                  | 功能                                                     |
| --------------------- | -------------------------------------------------------- |
| `--only user-prompts` | 只导出用户提示词，适用于 `jsonl` 和 `md`                 |
| `--redact`            | 对 API 密钥、令牌和凭证进行脱敏；`trace` 默认启用脱敏    |
| `--lineage logical`   | 将压缩前后的谱系合并为一个逻辑会话，适用于 `md` 和 `qmd` |

```bash
hermes sessions export --format md --session-id <session_id>               # 将单个会话导出为 Markdown
hermes sessions export --format qmd --older-than 30                        # 将 30 天前的会话批量导出为 Quarto
```
...
hermes curator usage               # 查看全部技能的使用记录
```
...
| --------------------------- | -------------------------------------- | ----------------------------------- | ------------------------------------- |
| `pre_tool_call`             | 工具执行前                             | 阻止危险命令、检查参数、审计调用    | `block` 阻止；`approve` 人工审批      |
...
## 13.3 Profile 路由与聊天覆盖
Profile 路由根据消息的平台、服务器、聊天或线程标识，决定由哪个 Profile 处理消息。没有匹配路由的消息由启动该 Gateway 的 Profile 处理`。
...
| 配置项      | 匹配范围                                 |
| ----------- | ---------------------------------------- |
| `platform`  | 消息平台，适用于所有 Gateway 平台适配器  |
| `guild_id`  | 服务器标识，主要用于 Discord             |
| `chat_id`   | 频道、群聊或私聊标识                     |
| `thread_id` | 线程或话题标识，需要消息平台提供对应信息 |

`channel_overrides` 可以按聊天或频道覆盖模型、提供商和系统提示词：
    ...
    <platform>:
      channel_overrides:
        "<chat_id>":
          provider: openai-codex
          model: gpt-5.5
          system_prompt: "负责处理支持请求"
```

`<platform>` 是消息平台名称，例如 `discord` 或 `telegram`；`<chat_id>` 是频道、群聊或私聊的实际 ID。

## 13.4 会话管理
Gateway 通过配置文件中的 `session_reset` 参数控制会话重置。默认使用 `mode: none`，不会自动结束当前会话。

将 `mode` 设置为 `idle`、`daily` 或 `both` 可以主动启用自动重置。`both` 会同时启用空闲重置和每日定时重置，并在任一条件满足时重置会话。

```yaml
# ~/.hermes/config.yaml
session_reset:
  mode: both          # 主动启用 idle 和 daily
...
顶层 Agent 调用 `delegate_task` 时默认后台运行，工具调用会立即返回包含运行状态、`delegation_id` 和 `live_transcripts` 在内的委派元信息。`live_transcripts` 为每个子 Agent 提供一个实时追加的转录文件，记录流式回复、工具调用和工具结果。

在另一个终端中查看子 Agent 的执行过程：

```bash
tail -f <transcript-path>
```

子 Agent 完成后会把结构化摘要送回对话；批量任务全部完成后按输入顺序合并返回。

Hermes 会持久化已经完成但尚未送回对话的后台委派结果，并在进程重启后继续投递。
