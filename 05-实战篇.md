...
publisher tools disable web browser code_execution
```

> **涉及功能**：Profile、Provider、SOUL.md、Toolsets、Skills

### 1.3 配置 Gateway 与 MCP
...
hermes config set mcp_servers.github.command "npx"
hermes config set mcp_servers.github.args '["-y", "@modelcontextprotocol/server-github"]'
```
...
# 为 Kanban 工作 profile 加载 worker skill
researcher skills list | grep kanban-worker || researcher skills reset kanban-worker --restore
writer skills list | grep kanban-worker || writer skills reset kanban-worker --restore
reviewer skills list | grep kanban-worker || reviewer skills reset kanban-worker --restore
publisher skills list | grep kanban-worker || publisher skills reset kanban-worker --restore
```
